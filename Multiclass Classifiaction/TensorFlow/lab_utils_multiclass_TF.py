...
dkcolors = cm.get_cmap("Paired")((1, 3, 7, 9, 5, 11))
ltcolors = cm.get_cmap("Paired")((0, 2, 6, 8, 4, 10))
...
def plt_mc_data(ax, X, y, classes, class_labels=None, map_in=cm.get_cmap("Paired"), legend=False, size=50, m='o',
                equal_xy=False):
    for i in range(classes):
        idx = np.where(y == i)
        col = len(idx[0]) * [i]
        label = class_labels[i] if class_labels else "c{}".format(i)
        ax.scatter(X[idx, 0], X[idx, 1], marker=m, color=map_in(col), s=size, label=label)
    if legend:
        ax.legend()
    if equal_xy:
        ax.axis("equal")
...
def plt_mc(X_train, y_train, classes, _):
    np.unique(y_train)
    fig, ax = plt.subplots(1, 1, figsize=(3, 3))
    fig.canvas.toolbar_visible = False
    fig.canvas.header_visible = False
    fig.canvas.footer_visible = False
    plt_mc_data(ax, X_train, y_train, classes, map_in=dkcolors_map, legend=True, size=50, equal_xy=False)
...
def plt_cat_mc(X_train, y_train, model, classes):
    ...
    fig.canvas.header_visible = False
    fig.canvas.footer_visible = False
    plt_mc_data(ax, X_train, y_train, classes, map_in=dkcolors_map, legend=True)
...
def plt_prob_z(ax, fwb, x0_rng=(-8, 8), x1_rng=(-5, 4)):
    ...
    pcm = ax.pcolormesh(tmp_x0, tmp_x1, z,
                        norm=colors.Normalize(vmin=np.amin(z), vmax=np.amax(z)),
...
def plt_layer_relu(X, Y, W1, b1, classes):
    ...
    for i in range(nunits):
        ...
        plt_mc_data(ax[i], X, Y, classes, map_in=dkcolors_map, legend=True, size=50, m='o')
...
def plt_output_layer_linear(X, Y, W, b, classes, x0_rng=None, x1_rng=None):
    ...
    for i, axi in enumerate(ax.flat):
        ...
        plt_mc_data(axi, X, Y, classes, map_in=dkcolors_map, legend=True, size=50, m='o')
