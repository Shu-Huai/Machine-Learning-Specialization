...
def mk_cost_lines(x, y, w, b, ax):
    ...
    for p in zip(x, y):
        ...
        ax.annotate(f'{c_p_txt:0.0f}', xy=cxy, xycoords='data', color=dlpurple, xytext=(5, 0),
...
def inbounds(a, b, xlim, ylim):
    ...
    ax, ay = a
    bx, by = b
    if (xlow < ax < xhigh) and (xlow < bx < xhigh) \
...
def plt_contour_wgrad(x, y, hist, ax, w_range=None, b_range=None, contours=None,
        ...
        if edist > resolution or point == hist[-1]:
            if inbounds(point, base, ax.get_xlim(), ax.get_ylim()):
                plt.annotate('', xy=point, xytext=base, xycoords='data',
                             arrowprops={'arrowstyle': '->', 'color': 'r', 'lw': 3}, va='center', ha='center')
            base = point
    return
...
def add_line(dj_dx, x1, y1, d, ax):
    ...
    ax.annotate(r"$\frac{\partial J}{\partial w}$ =%d" % dj_dx, fontsize=14, xy=(x1, y1), xycoords='data',
...
def run_gradient_descent(X, y, iterations=1000, alpha=1e-6):
    ...
    initial_b = 0
    w_out, b_out, hist_out = gradient_descent_houses(X, y, initial_w, initial_b, compute_cost, compute_gradient_matrix,
                                                     alpha, iterations)
